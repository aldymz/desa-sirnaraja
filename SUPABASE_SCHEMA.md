...
Jalankan SQL berikut di **Supabase SQL Editor** (`https://supabase.com/dashboard/project/<your-id>/sql`).

---

## 1. Tabel `desa_info` — Identitas & Kontak Desa
...
## 2. Tabel `potensi` — Potensi Unggulan Desa
...
-- Insert data awal dari dummyData
...
## 3. Tabel `aparatur` — Pemerintahan Desa
...
- Nama bucket: `potensi-images`  
- Visibility: **Public**

Kemudian tambahkan **RLS Policy** agar semua orang bisa membaca:

```sql
-- Policy: Allow public read on potensi-images
CREATE POLICY "Allow public read"
ON storage.objects FOR SELECT
USING (bucket_id = 'potensi-images');

-- Policy: Allow authenticated insert (opsional, untuk upload)
CREATE POLICY "Allow authenticated insert"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'potensi-images');
```

---

## 5. Row Level Security (RLS) — Akses Publik Read
...
-- Allow insert/delete for potensi (untuk fitur tambah/hapus dari website)
